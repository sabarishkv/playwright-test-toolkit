...
Try to avoid CSS selectors, XPath, or picking "the 3rd item in a list" (`.nth()`). These break easily whenever the page's design changes even slightly. Our linter will warn you if you use `.nth()`.
...
Never write `page.waitForTimeout(2000)` to "just wait a couple seconds and hope it's ready." Our linter blocks this. Instead, use an assertion that waits and retries automatically, like:
...
When you do this, always limit the mock to the exact URL you're faking (see `tests/mocking/posts-error-state.spec.ts` for an example). A mock that's too broad can accidentally catch requests you didn't mean to fake, hiding real problems instead of catching them.
