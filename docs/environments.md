...
**In this doc:** [How it works](#how-it-works) · [Adding a new environment](#adding-a-new-environment) · [Current settings](#what-settings-exist-today)
...
```mermaid
flowchart LR
    A[".env\n(base settings)"] --> C["Combined settings"]
    B[".env.NAME\n(overrides)"] --> C
    C --> D["Checked against envSchema"]
    D -->|All good| E["Ready to use in tests"]
    D -->|Something missing or wrong| F["Test run stops immediately\nwith a clear error"]
```
...
> **Tip:** if you add a new required setting, also add it to `envSchema` in `src/config/env.ts`. That way, a missing value is caught right away instead of causing a confusing error later on.
